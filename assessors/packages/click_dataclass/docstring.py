...
Credits: https://github.com/lebrice/SimpleParsing/
"""
...
class AttributeDocString:
    ...
    def aggregate(self) -> str:
        """Aggregate the comments into a single string."""
        comments = [self.comment_above, self.comment_inline, self.docstring_below]
        comments = [comment for comment in comments if comment]  # Filter empty
        if not comments:
            return ""
        return "\n".join(comments)
